...
class dd {
  ...
  constructor({ corpId, appKey, appSecret, agentId, custom, b1BizId, encodingAESKey, token }) {
    ...
    this.request.interceptors.response.use(
      response => {
        ...
        const { errcode, dentry } = dataAxios;
        if (errcode === 0) {
          return dataAxios;
        } else if (dentry) {
          return JSON.parse(dentry);
        }
  ...
  encrypt(text) {
    ...
    const $key = Buffer.from(this.custom ? this.appKey : this.corpId);
   ...
   * @param file
   */
  uploadSingle(file_size, file) {
    return new Promise((resolve, reject) => {
      this.request({
        url: 'file/upload/single',
        method: 'post',
        headers: file.getHeaders(),
        params: { file_size, agent_id: this.agentId },
        data: file,
      }).then(data => {
   ...
   */
  cspaceAdd(code, media_id, space_id, name) {
    return new Promise((resolve, reject) => {
      this.request({
        url: 'cspace/add',
        params: { code, media_id, space_id, name },
      }).then(dentry => {
        resolve({
          fileId: dentry.id,
          fileName: dentry.name,
          fileSize: dentry.size,
          fileType: dentry.extension,
          spaceId: dentry.spaceId + '',
          domain: dentry.cid,
        });
      }).catch(err => {
        reject(err);
   ...
   * @returns {Promise<unknown>}
   */
  getAllProcessTemplate(userid) {
    return new Promise((resolve, reject) => {
      this.request({
        url: 'topapi/process/template/manage/get',
        method: 'post',
        data: { userid },
      }).then(data => {
        resolve(data.result);
